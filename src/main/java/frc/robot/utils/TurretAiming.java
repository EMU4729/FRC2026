...
public class TurretAiming {
    ...
    public static final Transform3d turretOffset = new Transform3d(new Translation3d(0.3, 0.3, 0.4), new Rotation3d()); // TODO
                                                                                                                        // VERIFIY
                                                                                                                        // THIS

    public static Transform2d getTargetTransform(Translation2d target) {
        Pose2d turretPose = getTurretPose2d();
        Rotation2d targetYaw = target.minus(turretPose.getTranslation()).getAngle();
        return new Transform2d(
                turretPose,
                new Pose2d(target, new Rotation2d(targetYaw.getRadians())));
    }
    ...
    public static TurretState calcState(List<DistanceSample> sampleList, Translation2d target) {
        ...
        final var targetTransform = getTargetTransform(target);
        ...
        return new TurretState(Radians.of(targetTransform.getRotation().getRadians()), interpolatedSample.power(),
    ...
    private static DistanceSample interpolateDistance(List<DistanceSample> sampleList, Distance distance) {
        ...
        // extra checks are not wrong but as this list is constance we can be sure of
        // these by other means and not pay the cost to check every frame
        ...
        if (targetMeters >= upper.distance().in(Meters))
            return upper;
        ...
        double interpolatedPower = MathUtil.interpolate(lower.power().in(MetersPerSecond),
                upper.power().in(MetersPerSecond), t);
    ...
    public static Optional<AprilTag> getBestTarget(Pose2d robotPose) {
        ...
        for (AprilTag tag : fieldLayout.getTags()) {
            Translation2d tagToRobotTrans = robotPose.getTranslation()
                    .minus(tag.pose.getTranslation().toTranslation2d());
